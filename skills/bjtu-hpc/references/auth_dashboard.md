...
1. Refresh or save the portal token if `~/.bjtu_hpc_token` is missing or stale:
   ...
   ```
   To move aliases to another controller, read `environment_setup.md` and use `hpc_accounts.py export-json` / `import-json`. Metadata-only export is the default. `--include-tokens` or `--include-credentials` automatically wraps the complete payload in a passphrase-protected scrypt + AES-256-GCM JSON envelope. Enter the passphrase interactively during import, transfer file and passphrase separately, and delete migration artifacts after validation. Browser profiles and cookies are never exported.
