...
Use the helper scripts in the local `<SLURM_DIR>` workspace as the canonical interface to the BJTU HPC portal.
...
"$HPC_PYTHON" hpc_accounts.py export-json <private.json> \
  --include-tokens --include-credentials
"$HPC_PYTHON" hpc_accounts.py import-json <private.json> \
