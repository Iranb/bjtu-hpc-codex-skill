...
Controller Python contract: all local BJTU helper commands must use `HPC_PYTHON=<PYTHON3.12>`, which has Paramiko installed. Do not invoke a helper with bare `python3`; the macOS system interpreter is Python 3.9 and is unsupported. Any legacy `python3` command shown in a reference below means `"$HPC_PYTHON"`.
